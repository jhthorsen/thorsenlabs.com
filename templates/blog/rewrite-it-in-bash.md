...
```text
$ tt sync ssh.example.com:~/.local/share/battape/battape.sqlite
$ tt sync --pull ssh.example.com:~/.local/share/battape/battape.sqlite
```

The first command pushes this machine's database to `laptop`; the second pulls from it. Sync uses a SQLite backup and row IDs. If the same ID has conflicting data from another host, it reports the conflict instead of overwriting it silently.
...
That is the appeal for me: the daily workflow is useful immediately, and the source is close by when I want to change it.
